...
public class HuffCompress {
    ...
    public LinkedList<HuffManBean> list = new LinkedList<HuffManBean>();
    //统计次数
    ...
    //构造哈夫曼树
    public HuffManBean createTree(){
        //将次数作为权值构造森林
        for (int i = 0; i < times.length; i++) {
            if(times[i]!=0){
                HuffManBean node = new HuffManBean(times[i],i);
                //将构造好的节点加入到容器中的正确位置
        ...
        while(list.size()>1) {
            //获取容器中第一个元素（权值最小的节点）
            HuffManBean firstNode =list.removeFirst();
            //获取中新的第一个元素，原来的第一个元素已经被移除了（权值次小的节点）
            HuffManBean secondNode =list.removeFirst();
            //将权值最小的两个节点构造成父节点
            HuffManBean fatherNode =
                    new HuffManBean(firstNode.getData()+secondNode.getData(),-1);
    ...
    //利用前序遍历获取编码表
    public void getHuffmCode(HuffManBean root,String code){
        //往左走，哈夫曼编码加0
...
//          fos.flush();
    ...
    //插入元素位置的索引
    public int getIndex(HuffManBean node) {
